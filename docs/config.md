...
- The `$CODEX_HOME/config.toml` configuration file where the `CODEX_HOME` environment value defaults to `~/.codex`. (Codex stores logs and other state under `$CODEX_HOME`.)
